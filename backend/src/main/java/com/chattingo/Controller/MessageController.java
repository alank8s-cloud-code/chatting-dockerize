...
public class MessageController {
    ...
    @org.springframework.web.bind.annotation.PutMapping("/seen/{chatId}")
    public ResponseEntity<ApiResponse> markSeenHandler(@PathVariable Integer chatId,
            @RequestHeader("Authorization") String jwt) throws UserException, ChatException {

        User user = this.userService.findUserProfile(jwt);

        this.messageService.markMessagesAsSeen(chatId, user);

        ApiResponse res = new ApiResponse("Marked as seen", true);
